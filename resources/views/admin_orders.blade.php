    ...
    <div class="container mx-auto p-6">
        <div class="flex justify-between items-center mb-6">
            ...
            <form action="{{ route('admin.logout') }}" method="POST">
                @csrf
                {{-- Tombol logout kembali ke /admin --}}
                <button type="submit"
                        ...
                        @forelse ($orders as $order)
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    ...
                                    <span class="text-xs text-gray-400">{{ $order->nim }}</span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ number_format($order->duration_seconds) }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-green-600 font-bold">Rp{{ number_format($order->gross_amount, 0, ',', '.') }}</td>
